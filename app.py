    ...
    with ApiClient(line_configuration) as api_client:
        ...
        try:
                ...
                if group_id in messages:
                    ...
                    line_bot_api.reply_message(
                        ReplyMessageRequest(
                            ...
                            messages=[TextMessage(text="ขุนพระรีเซ็ตข้อมูลสำหรับกลุ่มนี้แล้วครับ! 🗑️")]
                        )
                    )
                return
            ...
            if user_message.upper().startswith(config.CHAT_SUMMARY_TRIGGER):
                ...
                if output:
                    ...
                    line_bot_api.reply_message(
                        ReplyMessageRequest(
                            reply_token=event.reply_token,
                            messages=[TextMessage(text="อ่า... ขุนพระ ไม่เห็นมีข้อความใหม่ให้สรุปแล้วนะครับ 😅\nไว้คุยกันเพิ่ม ค่อยเรียก 'ขุนพระ' ใหม่นะคับ")]
                        )
                    )
            ...
            line_bot_api.reply_message(
                ReplyMessageRequest(
                    reply_token=event.reply_token,
                    messages=[TextMessage(text="ขุนพระ! ตอนนี้ผมเจอปัญหาทางเทคนิค 😭 กรุณารอ @Por มาแก้ทีนะจ้า")]
                )
            )
